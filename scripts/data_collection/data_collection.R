...
#   mutate(Tumor_type = "inflammatory colorectal polyp", Status = NA, Symbol = NA, Sample_status = NA, Disease_stage = NA, Survival_status = NA, Tumor_grade = NA)
...
#   filter(Assay.Type == "WXS" & LibraryLayout == "PAIRED" & Organism == "Canis lupus familiaris") %>%
#   select(Run,BioProject,BioSample, Sample.Name, BREED, Platform,ReleaseDate, sex,Tissue, Library.Name) %>%
#   rename(Run_ID = Run, Sample_ID = BioSample, Case_ID = Sample.Name, Breed = BREED, Sex = sex, Tumor_type = Library.Name) %>%
#   mutate(Organization = "Ghent University", Status = NA, Symbol = NA, Sample_status = NA, Age = NA, Disease_stage = NA, Survival_status = NA, Tumor_grade = NA)
...
#   select(Run,BioProject,BioSample, Sample.Name, BREED, Platform, ReleaseDate, Age, sex, Tissue) %>%
#   rename(Run_ID = Run, Sample_ID = BioSample, Case_ID = Sample.Name, Breed = BREED, Sex = sex) %>%
#   mutate(Tumor_type = "Not Tumor", Organization = "University of Helsinki", Status = NA, Sample_status = NA, Symbol = NA, Survival_status = NA, Tumor_grade = NA,Disease_stage = NA)
