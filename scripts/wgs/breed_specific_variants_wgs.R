...
if (length(args) < 7) {
  stop("Usage: Rscript --vanilla breed_specific_variants_wgs.R <build_meta_data_code_path> <VAF_input_file> <unique_variants_output_file> <enriched_variants_output_file> <specific_variants_output_file> <meta_data_file> <breeds_list>", call.=FALSE)
...
residue_column_count <- 4; # number of columns describing each variant
...
# no gene or protein data available
variant_name_to_output <- function(variant_name) {
  variant_tokens <- strsplit(variant_name, fixed=TRUE, split="_")[[1]];
  gene <- "None";
  locus <- paste(variant_tokens[1], variant_tokens[2], sep=":");
  mutation <- paste(variant_tokens[3], variant_tokens[4], sep=">");
  protein <- "None";
  return(paste(gene, locus, mutation, protein, sep="\t"));
}

# now reading VAF values for normal samples
## note that positions that has two or more alt variants are ignored!
